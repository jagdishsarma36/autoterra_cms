...
<!-- cta section -->
@php
    $cta = pageContentJson('pro_spatial', 'spatial.cta');
@endphp
...
@section('scripts')
<script>
// Scroll spy – auto-highlight active tab on scroll
(function() {
    ...
    tabs.forEach(tab => {
        ...
        if (!target) return;
        const match = target.match(/scrollToModule\('([^']+)'/);
        if (match) {
    ...
    const tabsH = document.querySelector('.ps-tabs')?.offsetHeight || 0;
    const offset = navH + tabsH + 8;
    ...
    const observer = new IntersectionObserver((entries) => {
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (found) {
                    if (!active || entry.boundingClientRect.top < active.boundingClientRect.top) {
                        active = entry;
                    }
                }
            }
        ...
        if (active) {
            const found = sections.find(s => s.el === active.target);
            if (found) {
